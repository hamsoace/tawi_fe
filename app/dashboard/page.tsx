  ...
  const handleFilterChange = (key: keyof FilterState, value: string | number) => {
    setFilters(prev => {
      ...
      if (key === 'page') {
        ...
        // Type-safe way to update other filter properties
        if (key === 'range') {
          newState.range = value as 'day' | 'week' | 'month' | 'quarter' | 'year';
        } else if (key === 'search') {
          newState.search = value as string;
        } else if (key === 'pageSize') {
          newState.pageSize = value as number;
        }
        newState.page = 1;
      }
                    ...
                    >
                      <div>
                        <p className="text-sm text-gray-500">
                          {formatPhoneNumber(transaction.senderMsisdn)} →{" "}
